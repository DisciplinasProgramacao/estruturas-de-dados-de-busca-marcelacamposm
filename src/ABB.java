...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
    	...
    	Lista<V> listaRecortada = new Lista<>();
    	recortar(raiz, chaveDeOnde, chaveAteOnde, listaRecortada);
		return listaRecortada;
	}
	
	private void recortar(No<K, V> raizArvore, K chaveDeOnde, K chaveAteOnde, Lista<V> lista) {
		
		int comparacaoInicio, comparacaoFim;
		...
		if (raizArvore != null) {
			...
			if (comparacaoInicio > 0)
				recortar(raizArvore.getEsquerda(), chaveDeOnde, chaveAteOnde, lista);
			
			if (comparacaoInicio >= 0 && comparacaoFim <= 0)
				lista.inserir(raizArvore.getItem());
			
			if (comparacaoFim < 0)
				recortar(raizArvore.getDireita(), chaveDeOnde, chaveAteOnde, lista);
		}
	}
